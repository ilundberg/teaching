...
\begin{frame}{Observational example}{Zhou, Xiang. 2022. \bref{https://doi.org/10.1177/00491241221113876}{Attendance, Completion, and Heterogeneous Returns to College: A Causal Mediation Approach}. Sociological Methods and Research.}

\bgray{Research Question} \vskip .1in
How do college attendance ($A$)\\and completion ($M$)\\affect earnings ($Y$)?
...
\includegraphics[width = \textwidth]{figures/Zhou2022_Fig2}

\end{frame}

\begin{frame}{Observational example}{Zhou, Xiang. 2022. \bref{https://doi.org/10.1177/00491241221113876}{Attendance, Completion, and Heterogeneous Returns to College: A Causal Mediation Approach.} Sociological Methods and Research.}

The original paper uses Double Machine Learning \hfill (complicated) \\
We will discuss sequential $g$-estimation \hfill (simpler) \vskip .3in \pause
A good reference on sequential $g$-estimation is: \vskip .1in
Acharya, A., Blackwell, M., \& Sen, M. (2016). \bref{https://doi.org/10.1017/S0003055416000216}{Explaining causal findings without bias: Detecting and assessing direct effects.} American Political Science Review, 110(3), 512-529.

\end{frame}

\begin{frame}[t]{Estimation by sequential $g$-estimation} \vskip .2in
\begin{center}
...
\bgray{Step 1:} What outcome would have been realized at each $M = m$? \vskip .1in
...
\bgray{Step 2:}
Construct a \bgray{de-mediated outcome}
$$\tilde{Y} = Y - \gamma(X,A,M)$$
where the de-mediation function $\gamma$ is
$$\underbrace{\gamma(X,A,M)}_{\substack{\text{Not a function of $Z$}\\\text{See below}}} = \underbrace{\E(Y\mid X, A, Z, M) - \E(Y\mid X, A, Z, M = 0)}_\text{Causal effect of the factual mediator value $M$ vs 0}$$ \vskip .1in
...
\begin{frame}{A visual summary: Nonparametric sequential $g$-estimation}
...
\node<4-8> [anchor = north west, align = left] at (0,1.3) {We observe outcome means $\bar{Y}$ in each subgroup.\\We can now impute the outcome $Y^{A0}$ under $M = 0$\\in each stratum of $\{A,Z\}$.};
\node<9> [anchor = north west, align = left] at (0,1.3) {To focus on the effect of $A$, we now ignore $Z$.};
\node<10-13> [anchor = north west, align = left] at (0,1.3) {To focus on the effect of $A$, we now ignore $Z$.\\We have a weighted average over $Z\mid A = a$ for each $a$.\\Because the effect of $A$ is identified, $\underbrace{(Z\mid A = a)}_\text{Observational}\quad\sim \underbrace{(Z^a)}_\text{Interventional}$};
...
\onslide<3-4>{
\node at (.6,.95) {$M = 1$};
\draw (.5, .9) rectangle (.7, 1);
\node at (.6,.8) {$M = 0$};
\draw (.5, .7) rectangle (.7, .9);
}
\onslide<4>{
\draw (.7, .7) rectangle (.9, .9);
\draw (.7, .9) rectangle (.9, 1);
\node at (.8,.95) {$\bar{Y}$};
\node at (.8,.8) {$\bar{Y}$};
}
\onslide<3-5>{
