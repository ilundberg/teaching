...
\begin{quote}
A future treatment $F$ cannot affect an outcome $Y$.\vskip .1in \pause
Dependence of $F$ on $Y$ would seem to imply confounding. \pause
\end{quote} \vskip .1in
\bgray{Want to say:}\\If and only if $Y$ is conditionally related to $F$,\\then $T\rightarrow Y$ is confounded.\\(this will require additional assumptions)
\end{frame}

\begin{frame}{Nonparametric Example 1.}
{Want to say: If and only if $Y$ is conditionally related to $F$, then $T\rightarrow Y$ is confounded} \vskip .1in
%{Does a relationship between $Y$ and $F$ imply confounding?} \vskip .1in
...
$\bgreen{\checkmark}$ Consistent with what we want to say
%$$F\cancel\indep Y \mid \{T,X\} \qquad \rightarrow \qquad \{Y^0,Y^1\}\cancel\indep T\mid X$$ \vskip .2in
}
\end{frame}

\begin{frame}{Nonparametric results: Example 2.}
{Want to say: If and only if $Y$ is conditionally related to $F$, then $T\rightarrow Y$ is confounded} \vskip .1in
%{Does a relationship between $Y$ and $F$ imply confounding?} \vskip .1in
...
\node[anchor = north west] at (0,.65) {Result 12. (Requires Assumptions 2--3)};
...
\begin{frame}
Feedback from class: \bref{https://tinyurl.com/CausalQuestions}{tinyurl.com/CausalQuestions} \vskip .1in
No office hours today \vskip .3in

\bgray{Invitation:} Cornell Sociology Department Colloquium\vskip .05in
Friday, 3-4:15pm, Uris G08\vskip .05in
Felix Elwert, University of Wisconsin, Madison\\
\begin{quote}\small\normalfont Rearranging the Desk Chairs: A Large Randomized\\Field Experiment on the Effects of Close Contact on Interethnic Relations\end{quote}
